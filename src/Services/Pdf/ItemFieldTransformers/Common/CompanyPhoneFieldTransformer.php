...
class CompanyPhoneFieldTransformer implements ItemFieldTransformerInterface
{
    public function supports(
        string $transformer
    ): bool
    {
        return $transformer === CompanyPhoneFieldTransformer::class;
    }
    ...
    public function transform(object $entity, PdfField $pdfItemField, int $position): string
    {
        ...
        if (!$entity->getCompany()->getPhone()) {
            return '';
        }
        
        return 'Telefon: ' . $entity->getCompany()->getPhone();
    }
}
